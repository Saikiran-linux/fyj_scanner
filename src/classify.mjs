...
const NON_TARGET = [
  ['service_hospitality', /\b(dishwasher|busser|barista|bartender|bar ?back|waiter|waitress|wait\s?staff|\bserver\b|host(ess)?|line cook|prep cook|\bcook\b|\bchef\b|kitchen|catering|restaurant|food service|food (?:&|and) beverage|hospitality|housekeep|room attendant|valet|concierge|crew member|fast food|drive.?thru|spa therap)/i],
  ...
  ['clinical_healthcare', /\b(care assistant|caregiver|carer|\bcna\b|nursing assistant|home health|dental (assistant|nurse|hygienist)|dentist|chirurgien|medical assistant|phlebotom|physical therap|occupational therap|veterinar|\bvet\b|registered nurse|\brn\b|\blpn\b|\bcma\b|patient care|pharmacy technician|midwife|paramedic|\bemt\b|behavio.?r(al)? (health )?technician|\brbt\b|\baba\b (therap|provider)|direct support professional|optician|esthetician|massage therap|practical nurse|mental health therap|diversional therap)/i],
  ['skilled_trades', /\b(electrician|plumber|\bhvac\b|automotive|\bmechanic\b|welder|machinist|carpenter|painter|roofer|locksmith|appliance (repair|technician)|field service (technician|tech)|maintenance (technician|worker)|installer|lineman|millwright|fabricator|tire technician|diesel|auto body|reconditioning)/i],
  ['manual_labor', /\b(warehouse|forklift|laborer|labourer|order picker|\bpicker\b|\bpacker\b|loader|mover|\bdriver\b|delivery|courier|chauffeur|landscap|groundskeep|cleaner|cleaning|janitor|custodian|sanitation|production (worker|operative|associate)|assembler|machine operator|general labor|farm|harvest|distribution (technician|associate|worker))/i],
